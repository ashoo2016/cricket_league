...
class Webservice_model extends CI_Model 
{
    ...
    {  
        if($request['my_team'] == "1")
        {
            ...
            { 
                ...
                $this->db->select('my_team_player.is_vicecaptain,my_team_player.is_captain,my_team_player.id,my_team_player.my_team_id,my_team_player.player_id,my_team_player.player_id,players.teamid,my_team_player.is_select,players.credit_points,players.name,designation.title,designation.short_term,players.playing_status');
            ...
            else
            {
                ...
                $this->db->select('my_team_player.id,my_team_player.my_team_id,my_team_player.player_id,my_team_player.player_id,players.teamid,my_team_player.is_select,players.credit_points,players.name,designation.title,designation.short_term,players.playing_status');
                $this->db->from('my_team_player');  
                $this->db->join('players', 'players.id = my_team_player.player_id');
                $this->db->join('designation', 'players.designationid = designation.id');
                $this->db->join('my_team', 'my_team_player.my_team_id = my_team.id');
        ...
        else
        {
            if($request['designationid'] == "0")
            { 
                $this->db->select('match_players.id,match_players.matchid,match_players.teamid,match_players.playerid,match_players.is_select,team.team_name,team.team_image,team.team_short_name ,players.credit_points,players.name,designation.image,designation.title,designation.short_term,match_players.playing_status');
    ...
    //             ORDER BY rank + 0 ASC" ;
      ...
      $sql =    "SELECT `leaderboard`.`id`, `registration`.`image`, `leaderboard`.`teamid`, `leaderboard`.`rank`, `leaderboard`.`name`, `leaderboard`.`user_id`, `leaderboard`.`points`, `leaderboard`.`TeamName`
FROM registration 
JOIN leaderboard 
